...
class JSON_API_Post {
  ...
  function save($values = null) {
    ...
    if (!empty($values['author'])) {
	  ...
	  $wp_values['post_author_email'] = $author->email;
    }
  ...
  function import_wp_object($wp_post) {
    ...
    $this->set_value('excerpt', $wp_post->post_content);
      ...
      foreach ($wp_custom_fields as $key => $value) {
        if ($json_api->query->custom_fields) {
          if (in_array($key, $keys)) {
            if($key == 'images')
			{
				$images = $wp_custom_fields[$key];
				$this->custom_fields->$key = unserialize($images[0]);
			}
			else
			{
