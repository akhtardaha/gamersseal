<?php
    class Report{
        ...
        static function Update($data, $id){
            global $wpdb;
            apply_filters("order_status_completed",$id,$data['order_status']);
            $res=$wpdb->update("{$wpdb->prefix}mp_orders",$data,array('order_id'=>$id)); 
            //apply_filters("order_status_completed",$id);
            return $res;
        }
        ...
        static function UpdateOrderItems($cart_data, $id){
            ...
            if(!empty($cart_data))
                ...
                foreach($cart_data as $pid=>$cdt){
                    ...
                    $commission = wpmp_site_commission($post_author);
                    if($commission == "" || $commission < 0) $commission = 0;
                    
                    
                    $wpdb->insert("{$wpdb->prefix}mp_order_items",
                        array(
                            'oid'=>$id,
                            'pid'=>$pid,
                            'quantity'=>$quantity,
                            ...
                            if($settings['tax']['tax_rate']){
                                ...
                                foreach($settings['tax']['tax_rate'] as $key=> $rate){
                                    ...
                                    if($rate['tax_class']==$tax_class){
                                        $taxes=0;
                                        if(isset($bdata['shippingin']['country']) && in_array($bdata['shippingin']['country'], $rate['country'])){
        ...
        function GetAllOrders($qry="",$s=0, $l=20) {
            global $wpdb;
           // die('call');
            return $wpdb->get_results("Select order_status,date,uid,price,quantity,pid,user_nicename, p.post_title as post_title,p.ID,p.post_author as post_author from {$wpdb->prefix}mp_orders join {$wpdb->prefix}mp_order_items on order_id=oid

right join {$wpdb->prefix}posts p on p.ID=pid
join
{$wpdb->prefix}users u on u.ID=p.post_author $qry  limit $s,$l");
        }

        function totalOrders($qry=''){
            global $wpdb;
           return $wpdb->get_var("Select Count(*) from {$wpdb->prefix}mp_orders join {$wpdb->prefix}mp_order_items on order_id=oid

right join {$wpdb->prefix}posts p on p.ID=pid
join
{$wpdb->prefix}users u on u.ID=p.post_author $qry");
        }
        
        function totalSale($qry=''){
            global $wpdb;
           return $wpdb->get_var("Select Sum(price) from {$wpdb->prefix}mp_orders join {$wpdb->prefix}mp_order_items on order_id=oid
